...
- **LLM:** Gemini 2.5 Flash via Google AI Studio (`@google/genai`). Default model: `gemini-2.5-flash` (env-overridable via `GEMINI_MODEL`) for all four sub-features — multimodal, fast, and image input is flat-rated per tile. `gemini-2.5-pro` is the reach option for the KPI Q&A path if Flash's reasoning depth proves too shallow; `gemini-2.5-flash-lite` is the cheaper / higher-RPM option for development iteration. Use `responseSchema` to constrain output JSON to the Zod schema server-side; combined with Zod parsing on the client, schema drift becomes effectively impossible.
...
- **OCR:** Gemini 2.5 Flash vision on a PNG render of each invoice. Do not bolt on Tesseract or AWS Textract — the point is to show that a single multimodal-LLM call replaces a multi-tool OCR pipeline at a known cost.
