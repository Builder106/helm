...
**Note on the model question.** Helm has been through three provider commits — Anthropic Claude (dropped: API cost), Llama 4 Scout via Groq (dropped: 500K-tokens-per-day free-tier cap blocked the 200-invoice corpus at ~70 invoices), and now **Gemini 2.0 Flash via Google AI Studio**. Gemini wins because its free tier has no daily token cap, image input costs only ~258 tokens per PNG (vs. Groq's ~6,400), and the 1,500 RPD limit comfortably fits a full 200-invoice run. The `Extractor` interface in `back/src/ap/extraction.ts` is provider-agnostic by design; `extraction-groq.ts` remains in the repo as an alternative provider, and a Claude implementation can land alongside if the Claude-on-resume signal becomes load-bearing for a specific application.
...
- **LLM:** Gemini 2.0 Flash via Google AI Studio (`@google/genai`). Default model: `gemini-2.0-flash` for all four sub-features — multimodal, fast, and the free tier has no daily token cap. Gemini 2.5 Flash is the reach option if Helm needs deeper reasoning for the KPI Q&A path. Use `responseSchema` to constrain output JSON to the Zod schema server-side; combined with Zod parsing on the client, schema drift becomes effectively impossible.
...
- **OCR:** Gemini 2.0 Flash vision on a PNG render of each invoice. Do not bolt on Tesseract or AWS Textract — the point is to show that a single multimodal-LLM call replaces a multi-tool OCR pipeline at a known cost.
